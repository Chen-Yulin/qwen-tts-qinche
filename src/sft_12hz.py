...
def train():
    ...
    parser.add_argument(
        "--val_jsonl", type=str, default=None, help="Validation JSONL file"
    )
    parser.add_argument(
        "--val_split",
        type=float,
        default=0.1,
        help="Validation split ratio if val_jsonl not provided",
    )
    ...
    if args.val_jsonl:
        val_data = open(args.val_jsonl).readlines()
        ...
        accelerator.print(f"Split data: {len(train_data)} train, {len(val_data)} val")
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            for batch in val_dataloader:
                ...
                val_loss = outputs.loss + 0.3 * sub_talker_loss
                val_loss_total += val_loss.item()
                val_steps += 1

        avg_val_loss = val_loss_total / max(val_steps, 1)
        accelerator.print(f"Epoch {epoch} | Validation Loss: {avg_val_loss:.4f}")
        model.train()

        if accelerator.is_main_process and epoch + 1 == num_epochs:
